      ...
      messages: [
        {
          role: "system",
          content:
            "You are an AI digital twin assistant representing Jan Alcaide. Answer questions as if you are the person, speaking in first person about your background, skills, and experience. If the question is not directly related to the profile, still try to give a helpful response based on your background.",
        },
        {
          role: "user",
...
export function createDigitalTwinServer() {
  const server = new Server(
    ...
    {
      // Enable server capabilities required for tools and notifications
      capabilities: {
        tools: true,
        resources: true,
        prompts: true,
      ...
      tools: [
        {
          ...
          description:
            "Ask questions about Jan Alcaide's professional background, skills, projects, and experience. The AI will provide answers based on his profile information.",
          inputSchema: {
      ...
      try {
        ...
        const sources = searchResults
          .map(
            (r) =>
              `- ${r.title} (relevance: ${(
                (r.score / Math.max(...searchResults.map((x) => x.score), 1)) *
                100
              ).toFixed(0)}%)`
          )
          .join("\n");
